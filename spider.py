...
def getTitle(prob):
    ...
    try:
        ...
        title = text.split(
            '<title> {prob} '.format(prob=prob))[1].split(' - 洛谷</title>')[0]
        titleList[prob] = title
        with open('cache/title.yml', 'a+', encoding='utf-8') as file:
...
def init():
    global uid, config
    config = yaml.load(open('config.yml', 'r+', encoding='utf-8').read())
    uid = config['userid']
...
def getACList():
    text = requests.get(
        'https://www.luogu.org/space/show?uid=%d' % uid,
        cookies=cookies,
        headers=headers).text
    text = re.sub(r'<span style="display:none">[\s\S]*?</span>', '', text)
    text = re.sub(r'<h2>尝试过的题目</h2>[\s\S]*?</div>', '', text)
    find = re.findall(r'<a data-pjax href="/problem/show\?pid[\s\S]*?</a>',
                      text)
    prob = []
    for it in find:
...
def getTodoList():
    text = open('todo.list', 'r+', encoding='utf-8').read()
    text = re.sub(r'#[\s\S]*?\n', '', text)
...
def update():
    global titleList
    titleList = yaml.load(
        open('cache/title.yml', 'r+', encoding='utf-8').read())
    ac = getACList()
    todo = getTodoList()
    tableFile = open('web/table.html', 'w+', encoding='utf-8')
    for it in todo:
